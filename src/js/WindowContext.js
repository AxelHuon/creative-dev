...
export default class WindowContext {
    ...
    addScene(scene) { this.sceneCollection.push(scene) }

    update() { this.sceneCollection.forEach(s => { if (s.isVisible) s.update() }) }
    resize() { this.sceneCollection.forEach(s => { s.resize() }) }
    scroll() { this.sceneCollection.forEach(s => { s.scroll() }) }
    ...
    onDeviceAcceleration() {
        this.sceneCollection.forEach(s => { s.onDeviceAcceleration() })
    }
