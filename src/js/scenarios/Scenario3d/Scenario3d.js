...
import { randomRange } from "../../utils/math/MathUtils"
...
class TestSquareBubbles extends THREE.Mesh {
    constructor(radius, color) {
        ...
        const material_ = new THREE.MeshBasicMaterial({ color: new THREE.Color(color) })
...
export default class Scenario3d extends Scene3D {
    constructor(nBubble, id = "canvas-scene") {
        ...
        for (let i = 0; i < nBubble; i++) {
            const bubble_ = new TestSquareBubbles(this.radius, this.colors[i % this.colors.length])
            bubble_.setPosition(
                randomRange(-this.width / 2 + this.radius, this.width / 2 - this.radius),
                randomRange(-this.height / 2 + this.radius, this.height / 2 - this.radius)
            )
    ...
    addBubble(x, y) {
        const bubble_ = new TestSquareBubbles(this.radius, this.colors[Math.floor(this.colors.length * Math.random())])
    ...
    removeBubble(bubble) {
        ...
        /** remove from array */
        this.bubbles = this.bubbles.filter(b => { return b !== bubble }) // remove
    }
    ...
    */
    ...
    onDeviceAcceleration() {
        /** debug */
        let coordinates_ = ""
        coordinates_ = coordinates_.concat(this.acceleration.x.toFixed(2), ", ", this.acceleration.y.toFixed(2))
