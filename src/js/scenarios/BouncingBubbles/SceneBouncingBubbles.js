...
import { clamp, distance2d, randomRange } from "../../utils/math/MathUtils"
...
class Bubble {
    ...
    update(width, height, speed = 1) {
        /** gravity */
        this.gx = this.x < this.radius || this.x > width - this.radius ? 0 : this.gx
    ...
    constructor(nBubble, id = "canvas-scene") {
        ...
        if (this.debug.active) {
            ...
            this.debugFolder.add(this.params, 'radius', 0, 20).onChange(() => {
                if (!!this.bubbles) { this.bubbles.forEach(b => b.radius = this.params.radius) }
            })
            this.debugFolder.add(this.params, 'lineWidth', 1, 10)
            this.debugFolder.add(this.params, 'nBubbles', 1, 30).onFinishChange(() => { this.generateBubbles() })
